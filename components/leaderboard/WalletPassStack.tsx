...
const HEADER_PEEK_DEFAULT = 52;
const HEADER_HEIGHT_DEFAULT = 52;
const CARD_FULL_HEIGHT_DEFAULT = 204;

const HEADER_PEEK_COMPACT = 40;
const HEADER_HEIGHT_COMPACT = 40;
const CARD_FULL_HEIGHT_COMPACT = 184;
...
function PassHeader({
  ...
  return (
    <div
      className="flex shrink-0 items-center justify-between px-5 sm:px-6"
      style={{ ...headerStyle(player.rank), height: headerHeight }}
    >
...
function WalletPass({
  ...
  return (
    ...
    >
      <PassHeader player={player} headerHeight={headerHeight} />
...
function initialStackOrder(count: number) {
  return Array.from({ length: count }, (_, i) => count - 1 - i);
}

type WalletPassStackProps = {
  players: readonly LeaderboardPlayer[];
  compact?: boolean;
};

export function WalletPassStack({ players, compact = false }: WalletPassStackProps) {
  const headerPeek = compact ? HEADER_PEEK_COMPACT : HEADER_PEEK_DEFAULT;
  const headerHeight = compact ? HEADER_HEIGHT_COMPACT : HEADER_HEIGHT_DEFAULT;
  const cardFullHeight = compact
    ? CARD_FULL_HEIGHT_COMPACT
    : CARD_FULL_HEIGHT_DEFAULT;

  const [stackOrder, setStackOrder] = useState(() =>
    initialStackOrder(players.length),
  );

  const stackHeight = useMemo(
    () => (stackOrder.length - 1) * headerPeek + cardFullHeight,
    [stackOrder.length, headerPeek, cardFullHeight],
  );
          ...
          return (
            <motion.div
              ...
              style={{
                ...
                height: isTop ? cardFullHeight : headerHeight,
              }}
              animate={{ top: stackPosition * headerPeek }}
              transition={{
